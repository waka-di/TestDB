...
public class TestUserDAO {
	...
	String password = "";
	 public void select(String name,String password) {
		 DBConnector db = new DBConnector(); //↓この2行でDBと会話するためのコード。
		 Connection con = db.getConnection();
		 String sql ="select * from test_table where user_name=? and password=?";//?は都度違うデータ入れるとき使用
		 try {
			 PreparedStatement ps = con.prepareStatement(sql);//DBまで運んでくれる箱
			 ps.setString(1, name);
	 ...
	 public void selectAll() {
		 ...
		 try {
			 ...
			 ResultSet rs=ps.executeQuery();
			 while (rs.next()) {  //1行ずつ実行。データなくなったら終了
				 System.out.println(rs.getString("user_name"));
				 System.out.println(rs.getString("password"));
			 }
		 } 
